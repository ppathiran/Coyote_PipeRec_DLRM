    ...
    with record_function("DLRM forward"):
        if use_gpu:  # .cuda()
            ...
            if ndevices == 1:
                ...
                #)
                if lS_o is not None:
                    lS_o = lS_o.to(device) if not isinstance(lS_o, list) else [t.to(device) for t in lS_o]
...
class FPGADataset(torch.utils.data.IterableDataset):
    """
    Iterable dataset that overlaps FPGA preprocessing with GPU training.
    """
    def __init__(self, total_samples, batch_size, device_id=0):
        super().__init__()
        self.copy_times = [] 
        self.slice_times = []
        ...
        self.wrapper = fpga_p2p_pybind.FpgaP2PWrapper(batch_size)
        # two output buffers to support preprocessing-training overlap, starting with buffer idx = 0
        self.buffer_idx = 0 
        
        # generate synthetic dataset
        self.wrapper.generate_batch_data()

        # kick off first batch immediately
        self._prep_thread = threading.Thread(
            target=self._preprocess, args=(0, 0)
        )
    ...
    # performs preprocessing of one batch with alternating destination buffers
    def _preprocess(self, buf_idx, batch_idx):
        self.wrapper.preprocess_single_batch(buf_idx)

    # device-to-device copy of raw GPU pointer to a Pytorch tensor
    def _tensor_from_gpu_ptr(self, gpu_ptr):
        elems = self.batch_size * 48
        return wrap_ptr.copy_coyote_ptr(
            gpu_ptr, elems, self.device_id
        ).view(torch.float32).view(-1, 48)
    ...
    def __iter__(self):
        ...
        for b in range(self.num_batches):
            # wait until current batch ready
            self._prep_thread.join()
                        
            # launch next FPGA preprocessing unless last batch
            if b < self.num_batches - 1:
                nxt = 1 - self.buffer_idx
                self._prep_thread = threading.Thread(
                    target=self._preprocess, args=(nxt, b + 1)
                )
                self._prep_thread.start()
            
            # retrieve dst GPU ptr
            gpu_ptr = self.wrapper.get_dst_ptr(self.buffer_idx)

            # copy raw GPU ptr to Pytorch tensor
            start_copy_time = time.time()
            t = self._tensor_from_gpu_ptr(gpu_ptr)
            end_copy_time = time.time()
            copy_time = end_copy_time - start_copy_time
            self.copy_times.append(copy_time)

            # slice tensor to get label, dense and sparse features
            start_slice_time = time.time()
            y = t[:, 0].to(torch.float32).view(-1, 1)
            X_int = t[:, 1:14]                      
            X_cat = t[:, 16:42].to(torch.int32)
            lS_o = [torch.arange(0, self.batch_size, dtype=torch.long)] * 26
            lS_i = [X_cat[:, i] for i in range(26)]
            W = torch.ones_like(y, dtype=torch.float32)
            CBPP = None
            end_slice_time = time.time()
            slice_time = end_slice_time - start_slice_time
            self.slice_times.append(slice_time)

            yield X_int, lS_o, lS_i, y, W, CBPP
...
def run():
    ...
    elif args.data_generation == "fpga":
        total_samples = 45000000 # to match Criteo dataset; adjust if needed   
        batch_sz = args.mini_batch_size
        train_data = FPGADataset(total_samples, batch_sz, device_id=0)
        train_ld = torch.utils.data.DataLoader(
            train_data, batch_size=None, num_workers=0, pin_memory=False
        )
        test_ld = []
    ...
    with torch.autograd.profiler.profile(
        ...
        if not args.inference_only:
            ...
            total_lat, total_tput = train_data.wrapper.get_measurements()
            total_lat_s = total_lat / 1_000_000
            print(f"\nPreprocessing throughput:             {total_tput:,.1f} MB/s")
            print(f"Total time to preprocess:               {total_lat_s:,.6f} s")

            total_copy_time = sum(train_data.copy_times)
            print(f"Total time to copy (to a tensor):       {total_copy_time:.6f} s")
