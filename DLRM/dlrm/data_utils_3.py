...
def getCriteoAdData(
        ...
        if criteo_kaggle:
            ...
            if path.exists(datafile):
                ...
                with open(datafile, 'rb') as f:  # Open the file in binary read mode.
                    for file_id in range(days):
                        split_file_size = total_per_file[file_id]
                        # current_file_data = sif_output[file_id]
                        binary_data = f.read(split_file_size)
                        if not binary_data:  # If no data was read, end of file was reached.
                            break
                        if len(binary_data) != split_file_size:
                            print("Incomplete row of data encountered. Possible file corruption or unexpected end of file.")
                            break
                        format_string = f'<{split_file_size}I'  # Define the format string for little-endian           unsigned integers.
                        split_file_tuple = struct.unpack(format_string, binary_data)  # Unpack the binary data into          integers.
                        m = split_file_size // num_integers
                        sif_output[file_id] = [split_file_tuple[i * num_integers: (i + 1) * num_integers] for i in range(m)]
                ...
                #             print("Incomplete row of data encountered. Possible file corruption or unexpected end of file.")
                #             break
                #         format_string = f'<{num_integers}I'  # Define the format string for little-endian           unsigned integers.
                #         line = struct.unpack(format_string, binary_row)  # Unpack the binary data into          integers.
    ...
    def process_one_file(
        ...
        #!
            ...
            y[i] = target
            #! change to np.uint32
            X_int[i] = np.array(line[1:14], dtype=np.uint32)
            #!
            if max_ind_range > 0:
                X_cat[i] = np.array(
                    # list(map(lambda x: int(x, 16) % max_ind_range, line[14:])),
                    list(map(lambda x: x % max_ind_range, line[16:42])),
                    dtype=np.int32,
                )
            else:
                X_cat[i] = np.array(
                    # list(map(lambda x: int(x, 16), line[14:])), dtype=np.int32
                    list(map(lambda x: x, line[16:42])), dtype=np.int32
                )
