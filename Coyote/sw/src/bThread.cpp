...
void* bThread::initRDMA_GPU(uint32_t buffer_size, uint16_t port, const char* server_address) {
    if (server_address) {
        char* service;
        if (asprintf(&service, "%d", port) < 0) { throw std::runtime_error("asprintf() failed"); }
        ...
        void *mem = getMem({CoyoteAlloc::GPU, buffer_size, true});
        std::cout << "Allocate memory to GPU " << std::endl;
